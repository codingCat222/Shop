...
import { Search, Plus, ArrowLeft, Image as ImageIcon, Trash2, ShieldCheck, Landmark, MapPin, Truck, AlertCircle, Sparkles, CheckCircle2 } from 'lucide-react';
import { TradeItem, EscrowStatus, TradeType, TradeCategory, UserProfile } from '../types';
import { generatePickupCode, BuyerPickupCodeDisplay, SellerPickupCodeEntry, MAX_ATTEMPTS } from './EscrowPickupCode';
...
interface TradeViewProps {
  ...
  onUpdateTradeFields: (id: string, fields: Partial<TradeItem>) => void;
}

export default function TradeView({ trades, activeProfile, onCreateTrade, onUpdateTradeStatus, onUpdateTradeFields }: TradeViewProps) {
  ...
  // Always read the live trade from the trades array so persisted fields (pickupCode, attempts, buyerUsername) stay in sync
  ...
  // Anyone can list a trade, and anyone else can buy it — role is per-trade, not per-account.
  const isSeller = (trade: TradeItem) => activeProfile.username === trade.creatorUsername;
  const isBuyer = (trade: TradeItem) => !!trade.buyerUsername && activeProfile.username === trade.buyerUsername;
      ...
      {currentView === 'list' && (
        <>
          ...
          <motion.div
            ...
            className="flex-1 overflow-y-auto px-4 py-3 no-scrollbar space-y-3"
          >
            {filteredTrades.length > 0 ? (
              filteredTrades.map((trade) => (
                <motion.div
                  ...
                  className="p-4 bg-white border border-slate-100 rounded-lg shadow-xs hover:shadow-sm cursor-pointer transition-shadow flex items-start gap-3.5 relative overflow-hidden"
                >
        ...
        <form onSubmit={handleFormSubmit} className="flex-1 flex flex-col h-full overflow-hidden">
          ...
          >
            ...
            <motion.div variants={{ hidden: { opacity: 0, y: 12 }, show: { opacity: 1, y: 0 } }}>
              ...
              <div className="grid grid-cols-3 gap-2">
                <div className="aspect-square bg-white border-2 border-dashed border-slate-200 hover:border-purple-300 rounded-lg flex flex-col items-center justify-center text-slate-400 gap-1 cursor-pointer hover:bg-slate-50/50 transition-all">
              ...
              <div>
                ...
                <div className="relative">
                  <input
                    ...
                    className="w-full pl-11 pr-4 py-3 bg-white border border-slate-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-purple-600/15 focus:border-purple-600 transition-all shadow-xs font-mono"
                  />
          ...
          <div className="sticky bottom-0 left-0 right-0 p-4 bg-white border-t border-slate-100">
            <motion.button
              ...
              whileTap={{ scale: 0.98 }}
              type="submit"
              className="w-full py-3.5 bg-purple-600 hover:bg-purple-700 text-white font-sans font-bold text-sm rounded-lg shadow-lg shadow-purple-100 transition-colors cursor-pointer flex items-center justify-center gap-2"
            >
                ...
                isSeller(selectedTrade) ? (
                  ...
                  <button
                    onClick={() => {
                      ...
                      onUpdateTradeFields(selectedTrade.id, {
                    ...
                    className="w-full py-3 bg-purple-600 hover:bg-purple-700 text-white font-sans font-bold text-xs rounded-lg shadow-md cursor-pointer text-center"
                  >
                ...
                <div className="space-y-3">
                  {isSeller(selectedTrade) ? (
                    <SellerPickupCodeEntry
                      expectedCode={selectedTrade.pickupCode || ''}
                      attempts={selectedTrade.pickupAttempts || 0}
                      locked={(selectedTrade.pickupAttempts || 0) >= MAX_ATTEMPTS}
                      onAttempt={() => {
                        onUpdateTradeFields(selectedTrade.id, {
                          pickupAttempts: (selectedTrade.pickupAttempts || 0) + 1
                        });
                      }}
                      onVerified={() => {
                        onUpdateTradeFields(selectedTrade.id, {
                    ...
                    />
                  ) : isBuyer(selectedTrade) ? (
                    <BuyerPickupCodeDisplay code={selectedTrade.pickupCode || ''} />
                  ) : (
                    <div className="p-4 bg-slate-50 border border-slate-100 rounded-lg text-center">
                      <p className="text-xs font-sans text-slate-500">
                  ...
                  {(isSeller(selectedTrade) || isBuyer(selectedTrade)) && (
                    <button
                      onClick={() => {
